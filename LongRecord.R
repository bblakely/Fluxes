...
=======
sorg.yield.ustac<-as.numeric(sorghum.mgmt[4,4:5]); sorg.yield[1]<-sorg.yield.ustac[1]*1.08 #conversion: us tons mxg / ac -> .91 metric tons / us ton * 2.47ac/ha * .48tC / t sorghum -> tC/ha
...
=======
plot(maize.hvst.sum[samp]~c(maize$xlDateTime, maize.2020$xlDateTime)[samp], ylim=c(-35,35), col=col.zm[samp], pch='.', ylab="Cumulative NEE, tC ha-1", xlab=''); points(misc.hvst.sum[samp]~c(miscanthus$xlDateTime, miscanthus.2020$xlDateTime)[samp], col='light blue', pch='.')
points(sorg.hvst.sum[samp]~c(sorghum$xlDateTime,sorghum.2020$xlDateTime)[samp], col=col.sb[samp], pch='.')
abline(h=0)
legend(as.numeric(min(maize$xlDateTime)), 35, legend=c("maize", "soybean", "miscanthus", "sorghum"), col=c("orange","light green", "light blue", "forest green"), lwd=1)
>>>>>>> 1ce1509f830aa9d17823bb6d603b6b3d299bc9a1
