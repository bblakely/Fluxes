...
#preprocess to NAN vertical jumps (was plotting as point clusters that look like lines; this should allow plotting as lines):
exclvert<-function(tdat, ddat){

  ty<-as.numeric(format(tdat$xlDateTime, "%Y"))[samp]
  y<-unique(ty)
  yearstart<-match(y, ty) #index of Jan 1 midnight each year
  excl.raw<-sort(c(yearstart, yearstart-1)) #combine this with Dec 31 23:30 of year before
  excl<-excl.raw[2:length(excl.raw)]#removes first exclusionary value as it is the day before the record starts and therefore doesn't exist
...
misc.hvst.sum.plot<-exclvert(misc.merge, misc.hvst.sum)
misc.c.hvst.sum.plot<-exclvert(misc.c.merge, misc.c.hvst.sum)

maize.hvst.sum.plot<-exclvert(maize.merge, maize.hvst.sum)
maize.c.hvst.sum.plot<-exclvert(maize.c.merge, maize.c.hvst.sum)

np.hvst.sum.plot<-exclvert(nativeprairie, np.hvst.sum)
sw.hvst.sum.plot<-exclvert(switchgrass, switch.hvst.sum)
sorg.hvst.sum.plot<-exclvert(sorg.merge, sorg.hvst.sum)


# #Plotting
# par(mar=c(3,5,1,1))
# plot(maize.hvst.sum~decyr(maize.merge$xlDateTime), type='l', ylim=c(-45,40), col="white", ylab="Cumulative NBP, MgC ha-1", xlab='', cex=2, yaxt="n", xaxt = "n", cex.lab=2); 
...
# 
# lines(maize.hvst.sum.plot~decyr(maize.merge$xlDateTime)[samp], col=col.zm,  lwd=3)
# lines(misc.hvst.sum.plot~decyr(misc.merge$xlDateTime)[samp], col="blue", lwd=3)
# lines(maize.c.hvst.sum.plot~decyr(maize.c.merge$xlDateTime)[samp], col=col.zmc,  lwd=3)
# lines(misc.c.hvst.sum.plot~decyr(misc.c.merge$xlDateTime)[samp], col="light blue", lwd=3)
# 
# lines(sorg.hvst.sum.plot~decyr(sorg.merge$xlDateTime)[samp], col=col.sb,  lwd=3, lty=2)
# lines(sw.hvst.sum.plot~decyr(switchgrass$xlDateTime)[samp], col="pink", lwd=3, lty=5)
# lines(np.hvst.sum.plot~decyr(nativeprairie$xlDateTime)[samp], col="plum", lwd=3, lty=3)
# 
...
# 
# #left to do: get soy coloration in. Speed up plot time by sampling; make sure this works with exclvert
