...
ggplot(lidar.dat[lidar.dat$doy>192,]) +
  aes(x = row, y = range, fill = height) + #, colour = flood
  geom_tile(size = 1L) +
  scale_fill_viridis_c(option = "viridis") +
  #scale_color_viridis_c(option = "magma") +
  theme_minimal() +
...
#Height
boxplot(data=lidar.w, height~doy, col='red', ylim=c(0, 4))
boxplot(data=lidar.e, height~doy, add=TRUE, col='blue')
...
#Height
boxplot(data=sorg.lai[e.ind,], Height..m.~Day.Number, border='blue')
boxplot(data=sorg.lai[w.ind,], Height..m.~Day.Number, border='red', add=TRUE)
boxplot(data=sorg.lai[p.ind,], Height..m.~Day.Number, border='black', add=TRUE)
...
plot(manual.e, pch=3, col='blue', type='l', lty=2, ylim=c(0,7), xlim=c(190, 250), lwd=2, main='Height', ylab="Height", xlab="Day of Year")
abline(v=c(195, 210, 233, 249), lty=3)
lines(manual.w, col='red',lty=2, lwd=2)
lines(auto.e, col='blue', lwd=2)
lines(auto.w, col='red', lwd=2)
...
legend(190, 7,legend=c("manual,  flood", "automated, flood", "manual, nonflood", "automated, nonflood"), 
