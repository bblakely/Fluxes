...
og<-ggplot(subset(fluxes.merge, year%in%c(2008:2016))) +
  ...
  theme_minimal()+
  labs(y = "C flux, Mg C ha-1 y-1", x=NULL)+
  ylim(-5, 5)+
...
yield<-ggplot(subset(fluxes.merge.gm, year%in%c(2008:2022))) +
  ...
  geom_boxplot(shape = "circle", fill=colset.merge.gm)+
  theme_minimal()+
  labs(x = NULL, y = "C yield, Mg C ha-1 y-1")+
  ylim(0,8)+
  theme(axis.text=element_text(size=24),axis.title=element_text(size=30,face="bold"))
...
legend(7, 11, 
       legend=c(paste("Miscanthus 1:", round(mean(mgbrf.new, na.rm=TRUE), 2), "Wm-2"), paste("Miscanthus 2:", round(mean(mgcrf.new, na.rm=TRUE), 2), "Wm-2"), paste("Sorg. unadj.:", round(mean(sbrf.new.a, na.rm=TRUE), 2), "Wm-2"),paste("Sorghum:", round(mean(sbrf.new, na.rm=TRUE), 2), "Wm-2"), paste("Maize 2:", round(mean(zmrf.new, na.rm=TRUE), 2), "Wm-2")), 
